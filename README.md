...
Any products used for tracing should ideally be co-registered (one image registered to another means shifting one to match the other). Successfully registered means flipping from image to image, a building or a tree is aligned with itself and not shifting slightly across the products. If they are not, it is more critical that the work is done carefully to ensure errors are not inserted. It is fine to use other images that are not perfectly co-registered to develop a better context of the digital canopy, just take care to ensure that this is only used as an aid.
